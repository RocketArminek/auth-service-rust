    ...
    run_database_test_with_default(
        |c| async move {
            let user = User::now_with_email_and_password(
                ...
                .unwrap();
            c.user_repository.lock().await.save(&user).await.unwrap();
            let row = c.user_repository.lock().await.get_by_email(&user.email).await.unwrap();
    ...
    run_database_test_with_default(
        |c| async move {
            let user = User::now_with_email_and_password(
                ...
                .unwrap();
            c.user_repository.lock().await.save(&user).await.unwrap();
            c.user_repository.lock().await.delete_by_email(&user.email).await.unwrap();
            let row = c.user_repository.lock().await.get_by_email(&user.email).await;
...
async fn it_can_update_user_roles() {
    run_database_test_with_default(
        |c| async move {
            ...
            let updated_user = c.user_repository.lock().await.get_by_id(&user.id).await.unwrap();
...
async fn it_can_handle_multiple_roles() {
    run_database_test_with_default(
        |c| async move {
            ...
            let saved_user = c.user_repository.lock().await.get_by_id(&user.id).await.unwrap();
