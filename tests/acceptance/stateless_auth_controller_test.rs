...
async fn it_returns_not_found_if_user_does_not_exist() {
    run_integration_test_with_default(
        |c| async move {
            let response = c.server
...
async fn it_auto_updates_password_scheme() {
    run_integration_test(
        ...
        |c| async move {
            ...
            let found_user = c.user_repository.lock().await.get_by_id(&user.id).await.unwrap();
...
async fn it_refreshes_token() {
    ...
    run_integration_test(
        ...
        |c| async move {
            ...
            let body = response.json::<LoginResponse>();
            let exp = Utc::now().add(Duration::new(at_duration as i64, 0)
                .unwrap_or_default());
